...
def rescale_shift_matrix(mtr):
    ...
    positive is 0).'''
    mtr_cp = nmp.copy(mtr)
    least_neg = nmp.max(mtr_cp[mtr_cp<0])
    mtr_cp[mtr_cp<0]-=least_neg
    least_pos = nmp.min(mtr_cp[mtr_cp>0])
...
def model_average_pwm_geometric(lasso_component, de_component,
                      binding_strengths, 
                      constants = {"quadrant I":3, "quadrant II":1, 
                                   "quadrant III":1, "quadrant IV":1,
                                   "B":1, "D":2,
                                   "Cb":0.1, "Cd":0.01}):
    ''' Performs model averaging using '''
    averaged = model_average_np(lasso_component, de_component, 
                                constants = constants)
    pwm_averaged = nmp.zeros(nmp.shape(averaged))
    for j, row in enumerate(averaged):
        for i, _ in enumerate(row):
            pwm_averaged[j, i] = list_geometric((averaged[j, i], 
